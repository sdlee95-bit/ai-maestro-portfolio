...
- 교육 유형에 맞는 서식을 알아서 고른다 (집합·혼합 → D안, 이러닝 → E안)
...
| 2026년 3월 — 집합교육 (D안) | 2026년 5월 — 이러닝 (E안) |
|---|---|
| ![집합](preview/02-집합교육형-2026년3월.jpg) | ![이러닝](preview/03-이러닝형-2026년5월.png) |
| 대면 장소 · 전달교육 참여 인정 | 기간 · 나라배움터 주소 · 강좌 2개 · 진도율 · 수강 절차 |

**이러닝은 서식을 따로 만들었다.** 연 19회 중 9회가 이러닝인데 정보 구조가 다르다 —
하루가 아니라 **기간**, 장소가 아니라 **수강처(URL)**, 참여율이 아니라 **진도율**,
그리고 **강좌가 2개**다. 집합교육용 서식에 넣으면 강좌명과 인정시간이 통째로 빠진다.

E안은 그림을 쓰지 않는다. 실어야 할 정보가 많아 자리가 없고, 장식보다
「내가 무엇을 해야 하나」(수강 절차)가 쓸모 있다고 봤다. 덕분에 **세로형(메신저용)도
성립한다** — 잘릴 인물이 없다.

![이러닝 세로형](preview/06-이러닝-세로형.png)
...
**D안(집합·혼합)의 세로형에서 인물이 잘린다.** 하단 띠 겹침과 날짜 삐져나감은
고쳤지만, 가로 일러스트를 세로 틀에 맞추면 오른쪽 인물이 잘려 나간다. 세로가
필요하면 그림 없이 가거나 세로 비율로 일러스트를 다시 받아야 한다.
(E안은 그림을 안 써서 세로형이 정상 동작한다)

**강좌가 3개 이상인 달은 확인해 본 적이 없다.** 이러닝 9건이 모두 2강좌였다.

**원자료가 어긋나는 달이 있다.** 2026년 1월 계획안은 교육내용이 「폭력예방교육,
반부패·청렴교육」인데 강좌 표의 구분은 「통일교육 / 갈등관리교육」(전달 값)이다.
임의로 고치지 않고, 어긋나는 정보가 실린 포스터가 나가지 않도록 **기본값에서
파일을 만들지 않는다.**
...
1. D안 세로형(메신저용) 레이아웃 — 세로 비율 일러스트 필요
2. 강좌 3개 이상인 달 대응
3. 계획안이 HWPX 로 올 때 표 추출 확인 (지금은 PDF 로만 검증)
